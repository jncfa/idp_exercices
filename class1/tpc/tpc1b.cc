/**
 * Interação e Deteção de Partículas 2021/2022
 * José Faria <github.com/jncfa>
 * 
 * Trabalho: Random Number Generator with a Gaussian distribution using the Box-Muller method
 *  
*/
#define HEADER_NAME "Interação e Deteção de Partículas 2021/2022"
#define AUTHOR_NAME "José Faria <github.com/jncfa>"
#define WORK_DESCRIPTION "Random Number Generator with a Gaussian distribution using the Box-Muller method"

#include <iostream>
#include <fstream>
#include <stdlib.h>
#include <string.h>
#include <time.h>
#include <math.h>

/**
 * Helper function used to print help on the console.
*/
static void print_help(std::string name)
{
    std::cout << "Usage: " << name << " [mean] [stddev] [N] [seed] filename \n"
              << "Options:\n"
              << "\t- mean (Default = 0): Mean value of the Gaussian distribution.\n"
              << "\t- stddev (Default = 1): Standard deviation of the distribution.\n"
              << "\t- N (Default = 100): Amount of numbers to generate.\n"
              << "\t- seed (Default = Current time in seconds): Seed value for the C++ Standard Library RNG.\n"
              << "\t- filename: Name of the file to write the values to.\n"
              << "\nExample: " << name << " 0.0 1.0 100 0 values.log\n"
              << "To print help: " << name << " -h\n"
              << std::endl;
}

/**
 * Helper function used to parse strings into floating point values.
*/
bool to_double(std::string str, double& value){
    char* tempPtr;
    double tempValue = strtod(str.c_str(), &tempPtr);

    // Check if it parsed the entire string, return false if not
    if (tempPtr == str.c_str() + str.size()){
        value = tempValue;        
        return true;
    }
    else{
        return false;
    }
}

/**
 * Helper function used to parse strings into long integers.
*/
bool to_long(std::string str, long& value){
    char* tempPtr;
    long tempValue = strtol(str.c_str(), &tempPtr, 10);

    // Check if it parsed the entire string, return false if not
    if (tempPtr == str.c_str() + str.size()){
        value = tempValue;        
        return true;
    }
    else{
        return false;
    }
}

/**
 * Helper function used to parse strings into long integers.
*/
bool to_ulong(std::string str, unsigned long& value){
    char* tempPtr;
    unsigned long tempValue = strtol(str.c_str(), &tempPtr, 10);

    // Check if it parsed the entire string, return false if not
    if (tempPtr == str.c_str() + str.size()){
        value = tempValue;        
        return true;
    }
    else{
        return false;
    }
}

/**
 * Entry point of the program.
*/
int main(int argc, char *argv[])
{
    printf("%s\n", HEADER_NAME);
    printf("%s\n\n", AUTHOR_NAME);
    printf("Trabalho: %s\n\n", WORK_DESCRIPTION);

    // Default values
    double mean = 0;
    double stddev = 1;

    unsigned long seed = time(0);
    unsigned long numberCount = 100;
    std::string fileName = "";

    // Parse arguments passed to the program
    switch (argc)
    {
    case 2:
        // Print help
        if (!strcmp(argv[1], "-h"))
        {
            print_help(argv[0]);
            return EXIT_SUCCESS;
        }
        // Get filename
        else
        {
            fileName = argv[1];
        }
        break;
    case 3:
        // Print error message if failed to parse arguments
        if (!(to_double(argv[1], mean))){
            fprintf(stderr, "Error, could not parse the arguments. \n");
            print_help(argv[0]);
            return EXIT_FAILURE;
        }
        fileName = argv[2];
        break;
    case 4:
        // Print error message if failed to parse arguments
        if (!(to_double(argv[1], mean) && to_double(argv[2], stddev))){
            fprintf(stderr, "Error, could not parse the arguments. \n");
            print_help(argv[0]);
            return EXIT_FAILURE;
        }
        fileName = argv[3];
        break;
    
    case 5:
        // Print error message if failed to parse arguments
        if (!(to_double(argv[1], mean) && to_double(argv[2], stddev) && to_ulong(argv[3], numberCount))){
            fprintf(stderr, "Error, could not parse the arguments. \n");
            print_help(argv[0]);
            return EXIT_FAILURE;
        }

        fileName = argv[4];
        break;

    case 6:
        // Print error message if failed to parse arguments
        if (!(to_double(argv[1], mean) && to_double(argv[2], stddev) && to_ulong(argv[3], numberCount) && to_ulong(argv[4], seed))){
            fprintf(stderr, "Error, could not parse the arguments. \n");
            print_help(argv[0]);
            return EXIT_FAILURE;
        }

        fileName = argv[5];
        break;

    default:
        print_help(argv[0]);
        return EXIT_FAILURE;
    }

    // Print initial conditions
    printf("Initial Conditions:\n");
    printf("\t- Mean: %f\n", mean);
    printf("\t- Standard Deviation: %f\n", stddev);
    printf("\t- Number of random numbers: %lu\n", numberCount);
    printf("\t- Seed: %lu\n", seed);

    // Get file descriptor to write to file
    std::ofstream outFile(fileName);

    // Initialize the system generator
    srand(seed);

    // Iteratively generate the random numbers and write them to the file
    printf("\nWriting numbers to file '%s'...\n", fileName.c_str());
    double random1 = 0;
    double random2 = 0;
    double Z = 0;

    for (int i = 0; i < numberCount; i++)
    {
        // Generate random value and normalize it
        random1 = rand() / ((double)RAND_MAX + 1);
        
        // Generate random value and normalize it
        random2 = rand() / ((double)RAND_MAX + 1);

        // Obtain the random number via inverse transformation
        Z = pow((-2*log(random1)), 0.5) * cos(2*M_PI*random2);

        // Adjust distribution to have specified mean and standard deviation & write to the file
        outFile << mean + stddev * Z << std::endl;
    }

    // Flush and close file descriptor
    outFile.close();
    printf("Finished.\n");

    return EXIT_SUCCESS;
}
