...
//--------- Material definition ---------
  ...
  // Initialize the NIST material manager and search for soft tissue material (G4_TISSUE_SOFT_ICRP)
  G4NistManager* manager = G4NistManager::Instance();
  G4Material* softTissue = manager->FindOrBuildMaterial("G4_TISSUE_SOFT_ICRP");
...
//--------- Definitions of Solids, Logical Volumes, Physical Volumes ---------
  ...
  // Target
  //------------------------------  
  G4double targetThickness = 500.0*cm;
   
  G4Box* solidTarget = 
  		new G4Box("target",0.5*targetThickness,HalfWorldLength,HalfWorldLength);
  G4LogicalVolume* logicTarget = 
  		new G4LogicalVolume(solidTarget,softTissue,"Target",0,0,0);
